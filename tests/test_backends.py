...
def run_backend_test(backend_name: str, sample_rate: float, duration: float = 10.0):
    """
  ...
  python test_backends.py --all
    ...
    if args.all:
        ...
        for backend in backends:
            success = run_backend_test(backend, args.fs, args.duration)
            results.append((backend, success))
            time.sleep(1)  # Pause entre les tests
    else:
        # Tester un backend spécifique
        success = run_backend_test(args.backend, args.fs, args.duration)
